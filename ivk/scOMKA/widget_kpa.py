...
class KpaWidget(QDockWidget):
    ...
    transmiter_modulation = config.odict((0, 'данные ЗК'), (2, 'данные ЗК + дальнометрия'), (3, 'дальнометрия'), (4, 'технологический'), (5, 'технологический'))
    ...
    def __init__(self, parent, tabs_widget):
        ...
        lb.hbox(spacing=5).add(QLabel("КА ИД:")).add(self.label_ka_id).add(QLabel("Сеанс:")).add(self.label_is_receiving).stretch().up() \
          .hbox(spacing=5).add(QLabel("Получено сообщений от ИВК:")).add(self.label_count_msg_from_ivk).stretch().up() \
          .hbox(spacing=5).add(QLabel("Номер последнего сообщения:")).add(self.label_last_ivk_msg_number).stretch().up() \
          .hbox(spacing=5).add(QLabel("Литера:")).add(self.label_letter).add(QLabel("Очередь ЗК:")).add(self.label_queue).stretch().up() \
          ...
          .hbox(spacing=5).add(QLabel("Вход ПРМ:")).add(self.label_receiver_input).add(QLabel("Выход ПРД:")).add(self.label_transmiter_output).stretch().up() \
          ...
          .hbox(spacing=5).add(QLabel("Оценка скорости движения по КА:")).add(self.label_ka_estimate_speed).stretch().up() \
    ...
    def updateUi(self, kpa_responce):
        ...
        if kpa_responce.msg_type == KPAResponce.RESPONCES_FROM_KPA['ДИ_КПА']['msg_type']:
            ...
            if dikpa.receiver_speed in KpaWidget.receiver_speed:
                self.setLabelText(self.label_receiver_speed, '%d - %s' % (dikpa.receiver_speed, KpaWidget.receiver_speed[dikpa.receiver_speed]))
            if dikpa.transmiter_modulation in KpaWidget.transmiter_modulation:
                self.setLabelText(self.label_transmiter_modulation, '%d - %s' % (dikpa.transmiter_modulation, KpaWidget.transmiter_modulation[dikpa.transmiter_modulation]))
            if dikpa.receiver_input in KpaWidget.receiver_input:
                self.setLabelText(self.label_receiver_input, '%d - %s' % (dikpa.receiver_input, KpaWidget.receiver_input[dikpa.receiver_input]))
            if dikpa.transmiter_output in KpaWidget.transmiter_output:
                self.setLabelText(self.label_transmiter_output, '%d - %s' % (dikpa.transmiter_output, KpaWidget.transmiter_output[dikpa.transmiter_output]))
    ...
    def __saveDumpChecked(self, checked):
        ...
        self.dump_file_path = Path(os.getcwd()).parent.joinpath('ivk_dump', 'TMI_DUMP %s.bin' % datetime.now().strftime('%Y.%m.%d %H_%M_%S'))

    def __dumpSaveDialog(self):
        path = self.dump_file_path.parent.joinpath('TMI_DUMP %s.bin' % datetime.now().strftime('%Y.%m.%d %H_%M_%S'))
        filename = QFileDialog.getSaveFileName(self, 'Сохранить файл', str(path), 'Бинарные файлы (*.bin)')[0]  # @UndefinedVariable
        if filename != '':
