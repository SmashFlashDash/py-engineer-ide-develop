...
from PyQt5.QtWidgets import QMainWindow, QFrame, QWidget, QDockWidget, QShortcut, QAction, QBoxLayout, QPushButton, \
    QTabWidget, QTabBar
...
class MainWindow(QMainWindow):
    ...
    def initMenu(self):
        ...
        commands_console_widget_action.triggered.connect(
            lambda: self.openDock(self.commands_console_dock_widget, Qt.LeftDockWidgetArea))
        ...
        total_console_widget_action.triggered.connect(
            lambda: self.openDock(self.total_console_dock_widget, Qt.BottomDockWidgetArea))
        ...
        commands_widget_action.triggered.connect(
            lambda: self.openDock(self.commands_dock_widget, Qt.LeftDockWidgetArea))
        ...
        for dock in self.exchange_docks:
            ...
            action.triggered.connect(lambda qt_checked, dock=dock: self.openDock(dock,
                                                                                 Qt.RightDockWidgetArea))  # нормальное запоминания контекста в lambda
            widgets_menu.addAction(action)

    @staticmethod
    def createBasicActions(parent, add_new_action_func, open_action_func, save_action_func, save_as_action_func,
                           search_action_func, undo_action_func, redo_action_func, settings_action_func,
                           log_db_action_func, exit_action_func):
        ...
        return [add_new_action, open_action, save_action, save_as_action, search_action, undo_action, redo_action,
                settings_action, db_log_action, exit_action]
    ...
    def tabDragDrop(self, tab, from_tabs_widget, from_window, pos):
        ...
        tab.loading_widget = GifLabelSaveSpace('res/loading_orange_16.gif', visible=False, fix_w=16, fix_h=16)
    ...
    def saveSettings(self):
        ...
        for dock in self.available_docks:
            ...
            s.setValue('pos',
                       dock.mapToGlobal(QPoint(0, 0)) if dock.isFloating() else dock.pos() if dock.isVisible() else
                       dock.open_settings['pos'])
    ...
    def restoreSettings(self):
        ...
        for i in range(n):
            s.setArrayIndex(i)
            tab = self.tabs_widget.newTab(s.value('file', type=str), s.value('text', type=str),
                                          s.value('breakpoints', type=set), s.value('search_options', type=dict))
        ...
        for i in range(n):
            ...
            if s.value('maximized') is not None and s.value('maximized',
                                                            type=bool):  # only after size_ and pos_ are set
                frame.showMaximized()
            else:
                frame.show()
            ...
            for j in range(m):
                s.setArrayIndex(j)
                tab = frame.tabs_widget.newTab(s.value('file', type=str), s.value('text', type=str),
                                               s.value('breakpoints', type=set), s.value('search_options', type=dict))
                tab.restoreSettings(s.value('ui_settings', type=dict))
            s.endArray()
        ...
        for i in range(n):
            ...
            if s.contains('settings'):
                ...
                if not s.value('visible', type=bool):
                    dock.restoreSettings(
                        dock.open_settings['settings'])  # for case when we don't open that dock before closing app
        ...
        if self.save_tabs_on_close:
            while self.tabs_widget.count() > 0:
                if not self.tabs_widget.closeTab(self.tabs_widget.count() - 1, False, False, False):
                    event.ignore()
                    return
        if self in self.main_window.frames_stack:
            self.main_window.frames_stack.remove(self)
        if self in self.main_window.tabs_frames:
            self.main_window.tabs_frames.remove(self)
        super().closeEvent(event)
