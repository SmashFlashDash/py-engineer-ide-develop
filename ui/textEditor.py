...
from PyQt5.QtWidgets import QToolTip, QApplication, QListWidget, QListWidgetItem, QWidget, QPushButton, QBoxLayout, \
    QLabel, QTextEdit
...
class TextEditor(QsciScintilla):
    ...
    def __init__(self, parent, file, text, breakpoints, search_options, search_functions, modified_state_changed_func,
                 update_executor_breakpoint_func):
        ...
        self.search_options = search_options if search_options else {'text': '', 'replace': '', 're': False,
                                                                     'cs': False, 'wo': False, 'wrap': True}
    ...
    def initScintilla(self, text):
        ...
        self.setMarginMarkerMask(TextEditor.LINE_SELECTION_MARGIN,
                                 1 << TextEditor.CURRENT_LINE_MARKER | 1 << TextEditor.DEBUG_LINE_MARKER | 1 << TextEditor.ERROR_LINE_MARKER)
        ...
        self.setMarginMarkerMask(TextEditor.ERROR_WARNING_MARGIN,
                                 1 << TextEditor.ERROR_MARKER | 1 << TextEditor.WARNING_MARKER | 1 << TextEditor.ERROR_WARNING_MARKER)
    ...
    def checkModified(self):
        self.modified = not os.path.isfile(self.file) or self.text() != open(self.file, mode='r',
                                                                             encoding='utf-8').read()
    ...
    def __marginClicked(self, margin, line, modifiers):
        if self.markersAtLine(line) >> TextEditor.BREAKPOINT_MARKER & 1:
            ...
            if self.text(line).strip() == '':
                Commons.WarningBox("Ошибка добавляения точки остановки",
                                   "Добавление точки остановки к пустой строке невозможно", self)
            elif self.text(line).strip().startswith('@'):
                Commons.WarningBox("Ошибка добавляения точки остановки",
                                   "Добавление точки остановки к декоратору невозможно", self)
            elif self.text(line).strip().startswith('def') or self.text(line).strip().startswith('class'):
                Commons.WarningBox("Ошибка добавляения точки остановки",
                                   "Добавление точки остановки к определению невозможно", self)
            elif self.text(line).strip() == 'pass':
                Commons.WarningBox("Ошибка добавляения точки остановки",
                                   "Добавление точки остановки к пустой инструкции невозможно", self)
            elif self.text(line).strip().startswith('#') or self.text(line).strip().startswith('"""') or self.text(
                    line).strip().startswith("'''"):
                Commons.WarningBox("Ошибка добавляения точки остановки",
                                   "Добавление точки остановки к комментарию невозможно", self)
            else:
                self.markerAdd(line, TextEditor.BREAKPOINT_MARKER)
    ...
    def __dwellHandler(self, pos, x, y, dwell_start):
        if pos != -1 and dwell_start:
            ...
            if errors or warnings:  # using fact that empty iterable are False
                if errors:
                    message += '<span style="color: #a50303;">Ошибки:<br/>&nbsp;&nbsp;-%s</span>' % (
                        '<br/>&nbsp;&nbsp;-'.join(errors))
                if warnings:
                    message += '%s<span style="color: #a59422;">Предупреждения:<br/>&nbsp;&nbsp;-%s</span>' % (
                    '<br/>' if errors else '', '<br/>&nbsp;&nbsp;-'.join(warnings))
    ...
    def updateSearchIndicators(self, just_wipe=False):
        ...
        if not just_wipe and so['text']:
            ...
            while True:
                ...
                search_pos = self.SendScintilla(QsciScintilla.SCI_SEARCHINTARGET, len(so['text']),
                                                so['text'].encode('utf-8'))
                if search_pos == -1:
                    break
    ...
    def __lintingEnded(self, lints, line_correction, run_func, fail_func):
        ...
        for lint in lints:
            ...
            if isinstance(lint['column'], int) and lint['column'] < len(line):
                ...
                self.fillIndicatorRange(lint['line'], lint['column'], lint['line'], lint['column_to'],
                                        self.ERROR_INDICATOR if lint['type'] != 'warning' else self.WARNING_INDICATOR)
            else:
                self.fillIndicatorRange(lint['line'], 0, lint['line'], len(line),
                                        self.ERROR_INDICATOR if lint['type'] != 'warning' else self.WARNING_INDICATOR)
    ...
    def afterScriptEnd(self, error_line, error_text):
        if error_line != -1:
            ...
            self.__clearLineMarkers()
        if error_text:
            Commons.WarningBox('Найдены ошибки',
                               'В процессе выполнения произошла ошибка.<br>Сообщение: %s' % error_text, self)
        self.setReadOnly(False)
        self.executing = False
    ...
    def doComplete(self, completion=None):
        ...
        if completion.type == 'function' and not line_text.strip().startswith(
                'import') and not line_text.strip().startswith('from'):
    ...
    def __autocompleteCalculated(self, line, column, completions):
        ...
        for completion in completions:
            ...
            info_btn = QIconHoverValueButton(QIcon('res/info.png'), QIcon('res/info_hover.png'), value=item,
                                             tooltip='Информация')
