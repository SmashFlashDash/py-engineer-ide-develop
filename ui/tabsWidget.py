...
class TabsWidget(QTabWidget):
    ...
    def newTab(self, file='', text='', breakpoints=None, search_options=None):
        w = TabWidget(self, file, text, breakpoints, search_options, self.needToUpdateSearchIndicators,
    ...
    def closeTab(self, index, create_tab_if_empty=True, check_modified_on_save=True, lint_text_on_save=True):
        ...
        if w.modified() and w.text() != '':
            ...
            box_result = Commons.YesNoCancelBox('Файл не сохранен',
                                                'Закрываемый файл "%s" был изменен. Сохранить изменения?' % name, self)
    ...
    def openTab(self):
        filename = QFileDialog.getOpenFileName(self, 'Открыть файл', self.main_window.open_dir,
                                               'Файлы программ испытаний (*.ivkng);; файлы Python (*.py)')[0]  # @UndefinedVariable
        if filename != '':
    ...
    def saveTabAs(self, w=None, check_modified=True, lint_text=True):
        if w is None:
            w = self.currentWidget()
        filename = QFileDialog.getSaveFileName(self, 'Сохранить файл', self.main_window.save_dir,
                                               'Файлы программ испытаний (*.ivkng)')[0]  # @UndefinedVariable
        if filename != '':
    ...
    def needToUpdateSearchIndicators(self, text_edit):
        if self.currentWidget():
            return self.currentWidget().needToUpdateSearchIndicators(
                text_edit) and self.search_widget.isVisible() and not self.search_widget.refreshing
...
class IvkTabBar(QTabBar):
    def __init__(self, tabs_widget, main_window):
        ...
        lb = QBoxLayoutBuilder(self.drag_frame, QBoxLayout.LeftToRight, margins=(2, 2, 2, 2), spacing=2,
                               object_name='dragTabFrame')
